...
public class ComponentInputProcessor {
    ...
    public List<IOTypeDescriptor> outputPayloadFrom(ComponentOutputDescriptor output) {
    ...
    public Function<Suggestion, IOTypeDTO> mapper(TrieMapWrapper typeAndAndTries, ComponentOutputDescriptor output) {
        return suggestion -> {
            ...
            if (StringUtils.isNotBlank(orDefault.listItemType())) {
                ...
                if (suggestions.isEmpty()) {
                    ...
                    // Complex type
                    IOTypeDescriptor compute = compute(output, Collections.singletonList(orDefault.listItemType())).stream().findAny().get();
                ...
                if (suggestions.isEmpty()) {
                    ...
                    // Complex type
                    IOTypeDescriptor compute = compute(output, Collections.singletonList(suggestionType)).stream().findAny().get();
