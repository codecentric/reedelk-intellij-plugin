...
public class ConfigurationServiceImpl implements ConfigurationService {
    ...
    public ConfigurationServiceImpl(Module module) {
        this.module = module;
        this.publisher = module.getMessageBus().syncPublisher(Topics.TOPIC_CONFIG_CHANGE);
    }
    ...
    public void addConfiguration(@NotNull ConfigMetadata newConfig) {
        ...
        PluginModuleUtils.getConfigsDirectory(module).ifPresent(configsFolder ->
                WriteCommandAction.runWriteCommandAction(module.getProject(), () -> {
                    ...
                    if (configFile != null) {
                        PluginException exception = new PluginException(ReedelkBundle.message("config.error.add.file.exists.already", finalFileName));
                    ...
                    try {
                        ...
                        if (configDirectoryVf == null) {
                            PluginException exception = new PluginException(ReedelkBundle.message("config.error.add.config.dir.not.created", finalFileName));
                            publisher.onAddError(configTypeFullyQualifiedName, exception);
