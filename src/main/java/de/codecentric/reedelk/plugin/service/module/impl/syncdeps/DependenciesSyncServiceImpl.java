...
import de.codecentric.reedelk.plugin.commons.Versions;
import de.codecentric.reedelk.plugin.maven.MavenResolveGoal;
...
public class DependenciesSyncServiceImpl implements DependenciesSyncService {
    ...
    public void syncInstalledModules(String runtimeHostAddress, int runtimeHostPort, Consumer<Void> onDone) {
        PluginExecutors.run(module,
                ReedelkBundle.message("module.sync.task.title", module.getName()),
                indicator -> {
    ...
    private void installModuleArtifactIntoRuntime(String address, int port, final MavenArtifact artifact) {
        runtimeApiService().install(artifact.getFile().getPath(), address, port, new RuntimeApiService.OperationCallback() {
            @Override
            public void onSuccess() {
                String artifactId = artifact.getArtifactId();
                String artifactVersion = artifact.getVersion();
                LOG.info(ReedelkBundle.message("module.sync.module.deployed", artifactId, artifactVersion));
            }
            ...
            public void onError(Exception exception) {
                String artifactId = artifact.getArtifactId();
                String artifactVersion = artifact.getVersion();
                LOG.warn(ReedelkBundle.message("module.sync.module.deploy.error", artifactId, artifactVersion, exception.getMessage()));
            }
        });
    }
}
